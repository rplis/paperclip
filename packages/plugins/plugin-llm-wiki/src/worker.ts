...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    ctx.actions.register("queue-paperclip-ingestion-backfill", async (params) => {
      ...
      const projectId = sourceScopeKind === "selected_projects" ? projectIds[0] ?? null : null;
      const rootIssueId = sourceScopeKind === "root_issues" ? issueIds[0] ?? null : null;
      if (!projectId && !rootIssueId) {
        return {
      ...
      const workItem = await createPaperclipDistillationWorkItem(ctx, {
        ...
        priority: "low",
        idempotencyKey: stringField(params.idempotencyKey) ?? `profile-backfill:${idempotencyScope}:${backfillStartAt ?? "begin"}:${backfillEndAt ?? "now"}`,
